...
def main():
    ...
    #Read in our train data (observed sequences) 
    X_orig = []
    inp = open('SW1.txt')
    count_length = 0
    count_call = 0
    ...
    #Read in our test labels
    Z1 = []
    inp = open('SW2_labels_call_type.csv')
    for t in inp:
        for i in t.split(','):
            Z1.append(int(i))
    Z1 = np.array(Z1)
    ...
    #Read in our test lenghts
    L1 = []
    inp = open('SW2_labels_events.csv')
    for t in inp:
        for i in t.split(','):
            L1.append(int(i))
    L1 = np.array(L1)
    ...
    #Read in our test data (observed sequences) 
    X_orig1 = []
    inp = open('SW2.txt')
    count_length = 0
    count_call = 0
    ...
    for t in inp:
        ...
        X_call.append(int(Z[count_call]))
        X_call.append(int(total_index))
        X_orig1[-1].append(X_call)
        count_call += 1 
        L_curr -= 1
        L_orig = -1
        total_index += 1

    X_orig1 = np.array(X_orig1)
    ...
    with open('results.csv', 'wb') as csvfile:
	    ...
	    print(conf)
            ...
            for i in range(0, len(feature_labels)):
                writer.writerow([feature_labels[i], length_index[i], y_true[i], y_pred[i]])	
            writer.writerow("\n")
